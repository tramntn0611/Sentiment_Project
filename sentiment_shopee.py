...
if choice == 'Giới thiệu': 
    ...
    st.image('project-10.jpg')
    st.write("""
    - Trước tiên cần tiền xử lý dữ liệu Text, xây dựng tập luật và gắn nhãn cho các đánh giá
    - Sau đó huấn luyện mô hình, thực hiện các phương pháp cải thiện kết quả mô hình
    - Lựa chọn mô hình tốt nhất và tiến hành dự đoán cho toàn bộ dữ liệu""")

    st.subheader("Huấn luyện và cải thiện mô hình")
    ...
    st.write("""
    - LR_Over_2 (Logistic Regression với phương pháp Over-sampling (RandomOverSampling)) đem lại kết quả tốt nhất
    - Chỉ số Recall, Precision đều khá cao (0.7 ~ 0.8), F1-Score trên 0.8, AUC đạt 0.8976
    ...
    if res:
        ...
        if df_selection.empty:
            ...
            leftcol1, rightcol1 = st.columns([1, 1.5])
            with leftcol1:  
                st.markdown(
                f"""
                <div style="text-align: center;">
